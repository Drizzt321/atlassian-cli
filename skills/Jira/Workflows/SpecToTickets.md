...
- `artifacts` — list of file paths to attach to this ticket after creation, or `none`. Paths are relative to `spec_dir` (prefix `./`) when the file is at or below the spec directory, or absolute when outside it. Example:
  - `./mockup_upload_page.png`
  - `./templates/sample_input.csv`
  - `/home/user/shared-assets/brand-guide.pdf`
